...
def export_dff(options):
    """ Mass export each selected object to its own .dff file. """
    ...
    if not os.path.exists(dff_exporter.path):
        os.makedirs(dff_exporter.path)

    objects = [obj for obj in bpy.context.selected_objects if dff_exporter.is_selected(obj)]
    
    for obj in objects:
        # Create a unique filename for each object
        object_name = bpy.path.clean_name(obj.name)  # Clean the name to be file-system friendly
        filename = os.path.join(dff_exporter.path, f"{object_name}.dff")
        
        # Create a DFF exporter instance for each object
        dff_exporter.file_name = filename
        dff_exporter.dff = dff.dff()  # Initialize a new DFF object
        dff_exporter.export_objects([obj])
        
        # Write the file
        dff_exporter.dff.write_file(filename, dff_exporter.version)
